...
raise SystemExit("Deprecated: use test_buddys_vision_simple.py")
...
def test_buddys_vision():
    ...
    try:
        ...
        print(f"   • Links: {len(structure.get('stylesheets', []))}")
        ...
        if inspection.get('forms'):
            ...
            for i, form in enumerate(inspection['forms'][:3], 1):
                print(f"      {i}. {form.get('id', 'unnamed')} - {form.get('method', 'unknown').upper()}")
