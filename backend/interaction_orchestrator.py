...
from backend.action_readiness_engine import ActionReadinessEngine, IntentCandidate, ReadinessDecision
from backend.session_context import SessionContextManager, PendingClarification
...
class InteractionOrchestrator:
    ...
    def _get_artifact_chain(self, message: str, session_context_obj) -> Optional[List[Dict[str, Any]]]:
        ...
        """
        from backend.artifact_views import get_recent_artifacts
    ...
    def _answer_artifact_chaining(self, message: str, artifacts: List[Dict[str, Any]]) -> ResponseEnvelope:
        ...
        """
        from backend.artifact_views import (
    ...
    def _create_mission_from_readiness(
        ...
        """
        assert intent in {"extract", "navigate", "search"}, f"Invalid intent: {intent}"
        
        if intent in {"extract", "search"}:
            assert action_object, f"Missing action_object for {intent}"
            assert source_url, f"Missing source_url for {intent}"
    ...
    def process_message(
        ...
        """
        from backend.observability import DecisionTraceLogger
    ...
    def _handle_execute(self, intent: IntentClassification, message: str, session_id: str, context: Optional[Dict] = None, trace_id: Optional[str] = None, **kwargs) -> ResponseEnvelope:
        ...
        WIRING FIX 3: Check if this is a deterministic task (pure math) first.
        If so, answer immediately without creating a mission.
        """
        from backend.observability import DecisionTraceLogger
            ...
            if readiness.decision == ReadinessDecision.READY:
                ...
                if mission_draft:
                    ...
                    response = mission_proposal_response(
                        mission=mission_ref,
                        summary=f"Mission ready: {mission_draft.get('objective_description', '')}"
                    )
                    ...
                    # PHASE 3C: Register as pending mission for approval
