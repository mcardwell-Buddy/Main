...
export const generateApprovalContext = (hustle) => ({
  ...
  event_type: 'approval',
  timestamp: new Date().toISOString(),
  summary: `New hustle candidate: "${hustle.name || hustle.title || 'Untitled'}" — ${hustle.risk_level || 'unknown'} risk, $${Number(hustle.estimated_upside || 0).toLocaleString()} upside`,
  context: {
    hustle_id: hustle.id,
...
export const generateAlertContext = (alert) => ({
  ...
  event_type: 'alert',
  timestamp: new Date().toISOString(),
  summary: `Alert: ${alert.title || 'System Alert'} — ${alert.description || 'Details not available'}`,
  context: {
    alert_id: alert.id,
...
export const generateConfidenceContext = (signal) => ({
  ...
  event_type: 'learning',
  timestamp: new Date().toISOString(),
  summary: `Confidence update: ${signal.signal_type || 'unknown'} — confidence now ${Math.round((signal.confidence_score || 0) * 100)}%`,
  context: {
    signal_id: signal.id,
...
export const generateExecutionContext = (execution) => ({
  ...
  timestamp: new Date().toISOString(),
  summary: `Execution: ${execution.tool_name || 'unknown tool'} — ${execution.status || 'unknown'} status, took ${execution.duration_ms || 0}ms`,
  context: {
...
export const generateOpportunityContext = (opportunity) => ({
  ...
  timestamp: new Date().toISOString(),
  summary: `Income opportunity: ${opportunity.name || 'Unnamed'} — approved, potential revenue $${Number(opportunity.potential_revenue || 0).toLocaleString()}`,
  context: {
...
export const generateBuddyResponse = (context) => {
  ...
  const responses = {
    ...
    execution: `An execution has completed. **${summary}**\n\n**Status:** ${details.status}\n**Duration:** ${details.duration_ms}ms\n\n${details.status === 'FAILED' ? '❌ This execution failed. Would you like me to troubleshoot?' : '✅ Execution successful.'}\n\n**What\'s next?**`,
