...
def mission_plan_response(
    mission: MissionReference,
    mission_plan_dict: Dict[str, Any],
    approval_decision: Dict[str, Any]
) -> ResponseEnvelope:
    """
    Create a mission plan response showing planning phase results.
    
    NEW: Shows cost/duration estimates, tool selection, and approval recommendation.
    
    Args:
        mission: Mission reference 
        mission_plan_dict: MissionPlan.to_dict() output
        approval_decision: Decision from MissionApprovalService.evaluate_approval()
        
    Returns:
        ResponseEnvelope: Mission plan envelope with estimates
    """
    # Extract key planning info
    tool_name = mission_plan_dict.get('primary_tool', {}).get('tool_name', 'unknown')
    estimated_cost = mission_plan_dict.get('total_estimated_cost', 0.0)
    estimated_duration = mission_plan_dict.get('total_estimated_duration', 0.0)
    risk_level = mission_plan_dict.get('risk_level', 'MEDIUM')
    is_feasible = mission_plan_dict.get('is_feasible', False)
    recommendation = approval_decision.get('recommendation', 'REVIEW')
    
    # Build summary text
    summary_lines = [
        f"📋 Mission Ready for Approval",
        f"",
        f"🎯 Objective: {mission.objective_description}",
        f"🔧 Selected Tool: {tool_name}",
        f"💰 Estimated Cost: ${estimated_cost:.4f}",
        f"⏱️ Estimated Duration: {int(estimated_duration)} seconds",
    ...
    if not is_feasible:
        feasibility_issues = mission_plan_dict.get('feasibility_issues', [])
        if feasibility_issues:
            summary_lines.append(f"⚠️ Issues: {feasibility_issues[0]}")
    
    # Add alternative tools info
    alternatives = mission_plan_dict.get('alternative_tools', [])
    if alternatives:
        summary_lines.append(f"🔄 {len(alternatives)} alternative tools available")
    ...
    plan_artifact = Artifact(
        ...
        content={
            ...
            'alternative_tools_count': len(alternatives)
        },
        metadata={
            'awaiting_approval': True,
            'plan_source': 'mission_planning_phase',
            'approval_checks': approval_decision.get('checks', [])
        }
    )
    envelope.add_artifact(plan_artifact)
    
    # Set UI hints based on recommendation
    color_scheme = 'success' if recommendation == 'APPROVE' else ('warning' if recommendation == 'REVIEW' else 'error')
    icon = 'check-circle' if recommendation == 'APPROVE' else ('info' if recommendation == 'REVIEW' else 'alert-circle')
    
    envelope.set_ui_hints(UIHints(
        priority='normal',
        color_scheme=color_scheme,
        icon=icon,
        suggested_actions=['Approve', 'Review Alternatives', 'Reject'],
        layout='modal'
    ))
