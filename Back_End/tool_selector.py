...
Research target:""",
    ...
    def plan_tool_selection(
        ...
        """
        Select best tool AND rank alternatives with cost/duration estimates.
        
        Returns:
        {
            "primary_tool": ToolOption (as dict),
            "alternatives": [ToolOption, ToolOption, ...]  (as dicts),
            "selection_reasoning": str
        }
        """
        from Back_End.mission_plan import ToolOption
        
        # Step 1: Score all available tools
        all_scores = self.analyze_goal(objective_description)
        
        # Step 2: Overlay historical performance data
        for tool_name in all_scores.keys():
            perf = tracker.get_stats(tool_name, domain=domain)
            historical_score = self._calculate_performance_score(perf)
            
            # Combine pattern + historical (70% pattern, 30% history)
            combined = (all_scores[tool_name] * 0.7) + (historical_score * 0.3)
            all_scores[tool_name] = combined
        
        # Step 3: Rank by combined score
        ranked = sorted(all_scores.items(), key=lambda x: x[1], reverse=True)[:5]
        
        # Step 4: For each top tool, build ToolOption with cost/duration
        tool_options = []
        for tool_name, confidence in ranked:
            option = self._build_tool_option(
                tool_name,
                confidence,
        ...
        return {
            "primary_tool": tool_options[0].to_dict() if tool_options else None,
            "alternatives": [t.to_dict() for t in tool_options[1:4]],  # Top 3 alternatives
            "selection_reasoning": (
                f"Selected {tool_options[0].tool_name} based on pattern match "
                f"({tool_options[0].confidence:.1%}) + historical success"
            ) if tool_options else "No tools available"
        }
    
    def _calculate_performance_score(self, perf: Optional[Dict]) -> float:
        """Calculate performance score (0.0-1.0) from tracker stats"""
        if not perf or perf.get('total_calls', 0) == 0:
            return 0.5  # Default if no data
        
        success_rate = perf['successful_calls'] / perf['total_calls']
        avg_latency_ms = perf.get('avg_latency_ms', 1000)
        
        # 80% weight on success, 20% on speed
        return (success_rate * 0.8) + (1.0 - min(avg_latency_ms / 10000, 1.0)) * 0.2
    
    def _build_tool_option(
        self,
        tool_name: str,
        ...
        """Build a ToolOption with cost and duration estimates"""
        from Back_End.mission_plan import ToolOption
        
        # Get historical performance
        perf = tracker.get_stats(tool_name, domain=domain)
        success_rate = (perf['successful_calls'] / perf['total_calls']) if perf and perf.get('total_calls', 0) > 0 else 0.5
        avg_latency = perf.get('avg_latency_ms', 1000) if perf else 1000
        failures = perf.get('failure_modes', []) if perf else []
        
        # Calculate performance score (0.0-1.0)
        performance_score = success_rate * 0.8 + (1.0 - min(avg_latency / 10000, 1.0)) * 0.2
        
        # Combined score
        combined_score = (pattern_confidence * 0.7) + (performance_score * 0.3)
        
        # Estimate cost and duration
        cost_est, cost_reason = self._estimate_tool_cost(tool_name, constraints)
        duration_est, duration_reason = self._estimate_tool_duration(
            tool_name,
            avg_latency,
            constraints
        )
        ...
        # Reasoning
        reasoning = (
            f"Pattern: {pattern_confidence:.1%}. "
        ...
        return ToolOption(
            ...
            cost_rationale=cost_reason,
            estimated_duration_seconds=duration_est,
            duration_rationale=duration_reason,
            success_rate=success_rate,
            avg_latency_ms=avg_latency,
            failure_modes=failures[:3],  # Top 3 failure modes
            failures_in_last_10=len([f for f in failures if f])
        )
    
    def _estimate_tool_cost(self, tool_name: str, constraints: Dict) -> tuple:
        """Estimate API cost for a tool"""
        ...
        base_cost = tool_costs.get(tool_name, 0.0)
        
        # Multiply by estimated calls
        if tool_name in ['web_search', 'web_research']:
            target_count = constraints.get('target_count', 50)
            max_pages = constraints.get('max_pages', 5)
            estimated_calls = min(target_count / 10, max_pages)
            total_cost = base_cost * estimated_calls
            reason = f"~{estimated_calls:.0f} API calls × ${base_cost:.4f}"
        elif tool_name == 'web_extract':
            max_pages = constraints.get('max_pages', 5)
            total_cost = base_cost * max_pages
            reason = f"{max_pages} pages × ${base_cost:.4f}"
        else:
            total_cost = base_cost
            reason = "No API cost"
        
        return round(total_cost, 4), reason
    
    def _estimate_tool_duration(
        self,
        tool_name: str,
        ...
        base_duration_ms = max(avg_latency_ms, 500)
        
        # Add overhead per estimated calls
        if tool_name in ['web_search', 'web_research']:
            target_count = constraints.get('target_count', 50)
            calls = max(1, target_count / 10)
            total_ms = base_duration_ms * calls
            reason = f"~{calls:.0f} calls × {avg_latency_ms:.0f}ms avg"
        elif tool_name == 'web_extract':
            max_pages = constraints.get('max_pages', 5)
            total_ms = base_duration_ms * max_pages
            reason = f"{max_pages} pages × {avg_latency_ms:.0f}ms per page"
        else:
            total_ms = base_duration_ms
            reason = f"{avg_latency_ms:.0f}ms baseline"
        
        total_seconds = total_ms / 1000
        
        # Warn if exceeds limit
        if total_seconds > constraints.get('max_duration_seconds', 300):
            reason += f" [EXCEEDS {constraints.get('max_duration_seconds')}s limit]"
        
        return round(total_seconds, 1), reason
