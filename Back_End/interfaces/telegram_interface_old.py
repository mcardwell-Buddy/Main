...
from Back_End.conversation.intent_classifier import IntentClassifier, IntentType
...
class TelegramInterface:
    ...
    def generate_response(self, incoming_message: str, intent: Optional[IntentType] = None) -> str:
        """Generate a response based on message intent.
        
        Args:
            incoming_message: The original message text
            intent: Optional intent classification (will be computed if not provided)
        
        Returns:
            Response text appropriate for the intent
        """
        if intent is None:
            intent = self.intent_classifier.classify(incoming_message)
        
        # Intent-aware response generation
        if intent == "status_request":
            return "I'm online and ready to assist. What can I help with?"
        
        elif intent == "reflection":
            return "That's an interesting thought. I'm here to listen and discuss."
        
        elif intent == "exploration":
            return "I can help explore ideas and possibilities with you. What would you like to discuss?"
        
        elif intent == "potential_action":
            return "I understand. For actions like sending, searching, or creating, I'll need your approval before proceeding."
