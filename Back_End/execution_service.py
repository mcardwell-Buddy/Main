...
from Back_End.execution_learning_emitter import get_learning_emitter
from Back_End.artifact_writer import get_artifact_writer
...
from Back_End.cost_estimator import ServiceTier, ModelType
from Back_End.mission_control.mission_progress_tracker import MissionProgressTracker
from Back_End.progress_emitter import register_progress_emitter
...
URL:"""
    ...
    def execute_mission(self, mission_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Initialize progress tracker to emit real-time updates
            progress_tracker = MissionProgressTracker(mission_id=mission_id)
            
            # PHASE 1.3: Wire progress tracking to streaming events
            # This registers the progress emitter callback which:
            # - Emits real-time progress to WebSocket clients via StreamingEventEmitter
            # - Updates mission progress in Firebase (handled separately at end)
            register_progress_emitter(mission_id, progress_tracker)
            
            # PHASE 6: Register mission for WebSocket streaming
            # This ensures that when events are emitted during execution,
            # they are broadcast to all connected WebSocket clients in real-time
            StreamingEventsIntegration.register_mission_stream(mission_id)
            ...
            if current_status != "approved":
                progress_tracker.fail_step(f"Mission status is {current_status}")
                emitter.emit_mission_stop(
                    mission_id,
            ...
            if existing_executions > 0:
                progress_tracker.fail_step("Mission already executed")
                emitter.emit_mission_stop(
                    mission_id,
            ...
            progress_tracker.complete_step(f"Intent classified as {intent}")
            ...
            try:
                ...
                from Back_End.cost_estimator import MissionCost, ServiceCost
                ...
                if not budget_check['can_execute']:
                    ...
                    progress_tracker.fail_step(f"Budget exceeded: {budget_check['reason']}")
                    emitter.emit_mission_stop(
                        mission_id,
                ...
                progress_tracker.complete_step("Budget check passed")
            except Exception as budget_error:
                logger.error(f"[EXECUTION] Budget check error: {budget_error}")
                # Continue execution if budget check fails (failsafe)

            # STEP 5: Select tool (NEW: Use pre-planned tool from stored plan if available)
            progress_tracker.start_step(
            ...
            # CRITICAL FIX: Check for stored mission plan first
            stored_plan = self._get_stored_plan(mission_id)
            pre_selected_tool = False  # Track if we used planned tool
            
            if stored_plan and stored_plan.get('steps'):
                # Use the first planned tool (for single-step missions)
                # Multi-step execution will be Phase 2
                first_step = stored_plan['steps'][0]
                tool_name = first_step.get('selected_tool')
                confidence = first_step.get('tool_confidence', 0.9)
                pre_selected_tool = True  # Mark that we used a planned tool
                
                logger.info(
                    f"[EXECUTION] Using planned tool: {tool_name} "
                    f"(confidence: {confidence:.2f}, from stored plan)"
                )
                
                # Still need to generate tool_input using tool_selector's formatting
                _, tool_input, _ = tool_selector.select_tool(objective_description)
                
            else:
                # FALLBACK: Old behavior - dynamic tool selection
                logger.info(f"[EXECUTION] No stored plan found. Running dynamic tool selection...")
                tool_name, tool_input, confidence = tool_selector.select_tool(objective_description)

            if not tool_name or confidence < 0.15:
                progress_tracker.fail_step("Tool selection failed")
                emitter.emit_mission_stop(
                    mission_id,
            ...
            logger.info(f"[EXECUTION] Tool confirmed: {tool_name} (confidence: {confidence:.2f})")
            logger.info(f"[EXECUTION] Tool input: {tool_input[:100] if tool_input else 'N/A'}")
            ...
            progress_tracker.complete_step(f"Selected {tool_name}")
            ...
            if execution_success:
                progress_tracker.complete_step("Tool execution complete")
            else:
                progress_tracker.fail_step("Tool execution failed")

            # STEP 7: Write ONE execution result record (NEW: Include cost/duration variance)
            result_record = {
                'event_type': 'mission_executed',
                'mission_id': mission_id,
            ...
            # NEW: Add cost/duration variance for learning loop
            if execution_success and pre_selected_tool:
                estimated_cost = metadata.get('estimated_cost', 0.0) if isinstance(metadata, dict) else 0.0
                estimated_duration = metadata.get('estimated_duration', 0.0) if isinstance(metadata, dict) else 0.0
                
                if estimated_cost > 0 or estimated_duration > 0:
                    result_record['variance'] = {
                        'estimated_cost': estimated_cost,
                        'estimated_duration': estimated_duration,
                        # Actual values will be recorded by cost tracker separately
                        # This record is just for linking estimates to outcomes
                    }
                    logger.info(
                        f"[EXECUTION] Recorded variance data for learning: "
                        f"est_cost=${estimated_cost:.4f}, est_duration={estimated_duration}s"
                    )
            ...
            if execution_success:
                ...
                progress_tracker.fail_step("Execution failed")
            ...
            # PHASE 1.2: Persist progress to Firebase
            try:
                from Back_End.mission_store import get_mission_store
                mission_store = get_mission_store()
                mission_store.save_progress(mission_id, progress_tracker.to_dict())
                logger.info(f"[EXECUTION] Progress persisted to Firebase for {mission_id}")
            except Exception as e:
                logger.error(f"[EXECUTION] Failed to persist progress: {e}")
                # Non-blocking: continue even if persistence fails
            
            # PHASE 3: Record feedback for tool selection learning
            try:
                from Back_End.feedback_integrator import get_feedback_integrator
                
                integrator = get_feedback_integrator()
                
                # Extract mission type from objective
                mission_type = self._extract_mission_type(mission_record) if mission_record else "unknown"
                
                # Calculate execution time from progress tracker
                execution_time = progress_tracker.to_dict().get('elapsed_seconds', 0)
                
                # Record outcome
                integrator.record_mission_outcome(
                    mission_id=mission_id,
                    tool_used=tool_name,
                    ...
                    notes=f"Intent: {intent}" if intent else None
                )
                logger.info(
                    f"[EXECUTION] Feedback recorded for {mission_id}: "
                    f"{tool_name} on {mission_type} ({'✅' if execution_success else '❌'})"
                )
            except Exception as e:
                logger.warning(f"[EXECUTION] Failed to record feedback: {e}")
                # Non-blocking: continue even if feedback recording fails
            
            # PHASE 8: Log learning signal to Phase25
            try:
                bridge = get_phase25_bridge()
                mission_type = self._extract_mission_type(mission_record) if mission_record else "unknown"
                
                # Calculate signal confidence
                signal_confidence = confidence if execution_success else max(0, confidence - 0.2)
                
                # Determine signal type
                signal_type = "success" if execution_success else "failure"
                if execution_time > 60:
                    signal_type += "_slow"
                elif execution_time < 5:
                    signal_type += "_fast"
                
                # Log learning signal for Phase25
                bridge.log_learning_signal(
                    tool_name=tool_name,
                    signal_type=signal_type,
                    insight=f"Tool {tool_name} {'succeeded' if execution_success else 'failed'} "
                           f"on {mission_type} mission in {execution_time:.1f}s",
                    confidence=signal_confidence,
                    recommended_action="increase_priority" if execution_success else "decrease_priority",
                )
                logger.info(f"[EXECUTION] Phase25 learning signal logged for {mission_id}")
            except Exception as e:
                logger.debug(f"[EXECUTION] Phase25 signal logging skipped: {e}")
                # Non-blocking: Phase25 integration is optional
            ...
            return {
                ...
                'survey_pending': True  # PHASE 4: Survey available at /api/missions/{mission_id}/survey
            }

        except Exception as e:
            logger.error(f"[EXECUTION] Unexpected error during execution: {e}", exc_info=True)
            
            # CRITICAL FIX: Persist progress even on failure
            try:
                if 'progress_tracker' in locals():
                    from Back_End.mission_store import get_mission_store
                    mission_store = get_mission_store()
                    mission_store.save_progress(mission_id, progress_tracker.to_dict())
                    logger.info(f"[EXECUTION] Progress persisted to Firebase after error for {mission_id}")
            except Exception as persist_error:
                logger.error(f"[EXECUTION] Failed to persist progress after error: {persist_error}")
            ...
            # CRITICAL FIX: Always persist progress to Firebase, regardless of success/failure/early return
            try:
                if 'progress_tracker' in locals() and 'mission_id' in locals():
                    from Back_End.mission_store import get_mission_store
                    mission_store = get_mission_store()
                    mission_store.save_progress(mission_id, progress_tracker.to_dict())
                    logger.debug(f"[EXECUTION] Final progress persisted to Firebase for {mission_id}")
            except Exception as persist_error:
                logger.error(f"[EXECUTION] Failed to persist final progress: {persist_error}")
                # Non-blocking: don't fail execution if persistence fails
    ...
    def _get_stored_plan(self, mission_id: str) -> Optional[Dict[str, Any]]:
        """
        Retrieve the stored mission plan from mission_store.
        
        Returns plan_data dict with steps, costs, tools if available.
        This bridges the planning → execution gap by retrieving pre-planned tools.
        """
        try:
            store = get_mission_store()
            events = store.get_mission_events(mission_id)
            
            # Find the plan_created event
            for event in events:
                if event.event_type == 'mission_plan_created':
                    plan_data = event.metadata.get('plan_data')
                    if plan_data:
                        logger.info(f"[EXECUTION] Retrieved stored plan for {mission_id}: {len(plan_data.get('steps', []))} steps")
                        return plan_data
            
            logger.info(f"[EXECUTION] No stored plan found for {mission_id}, will use dynamic tool selection")
            return None
            
        except Exception as e:
            logger.error(f"[EXECUTION] Failed to retrieve plan for {mission_id}: {e}")
            return None
    ...
    def _write_execution_record(self, record: Dict[str, Any]) -> None:
        ...
        mission = Mission(
            ...
            metadata={
                ...
                'variance': record.get('variance'),  # NEW: Cost/duration variance for learning
...
"""
    ...
    def _extract_mission_type(self, mission_record: Optional[Dict[str, Any]]) -> str:
        """
        Extract mission type from mission record for feedback tracking.
        
        PHASE 3: Used to categorize missions for tool selection learning.
        ...
        try:
            objective = mission_record.get('mission_data', {}).get('goal', '')
            if not objective:
                objective = mission_record.get('objective', '')
            
            objective_lower = objective.lower()
            
            # Map keywords to mission types
            if any(word in objective_lower for word in ['tutorial', 'guide', 'how to', 'learn', 'teach']):
                return 'tutorial_search'
            elif any(word in objective_lower for word in ['extract', 'scrape', 'pull', 'get data']):
                return 'web_extract_data'
            elif any(word in objective_lower for word in ['calculate', 'math', 'compute', 'solve']):
                return 'calculate_math'
            elif any(word in objective_lower for word in ['navigate', 'go to', 'visit', 'browse']):
                return 'web_navigate'
            elif any(word in objective_lower for word in ['read', 'open', 'view', 'display']):
                return 'read_file'
            elif any(word in objective_lower for word in ['search', 'find', 'look up', 'research']):
                return 'web_search_general'
            else:
                return 'general_mission'
        except Exception as e:
            logger.warning(f"Failed to extract mission type: {e}")
            return "unknown"
